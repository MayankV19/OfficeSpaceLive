    ...
    if (typeof define === 'function' && define.amd) {
        ...
        var TableExport = function (selectors, options, isUpdate) {
            ...
            self.selectors.each(function () {
                var $el = $(this);
                $el.bootstrapTable('togglePagination');
                if (isUpdate) {
                ...
                var $rows = $el.find('tbody').find('tr'),
                    ...
                    fileName = ($el.attr('name') !== undefined ? $el.attr('name').replace("Table", "") : $el.attr('name')) + "_" + moment(d).format('MMDDYYYYHHMMSS'),//self.settings.fileName === "name" ? ($el.attr('name') ? $el.attr('name') : TableExport.prototype.defaultFileName) : self.settings.fileName,
                    exporters = {
                ...
                function checkCaption(exportButton) {
                    var elemBtn = $el.parent().parent().parent().find('.fixed-table-toolbar .columns-right');
                    var data = JSON.parse($(exportButton).attr("data-fileblob")).data.length;
                    if (data > 1) {
                        var exportBtnHTML = '<div class="export btn-group' + bootstrapSpacing + self.settings.position + '" style="display: inline-block;">' +
                                                '<button class="btn btn-default dropdown-toggle" aria-label="export type" title="Export data" data-toggle="dropdown" type="button" style="margin-top: -5px;"><i class="glyphicon glyphicon-export icon-share"></i> <span class="caret"></span></button>' +
                                                '<ul class="dropdown-menu" role="menu">' +
                                                    exportButton +
                                                '</ul>' +
                                            '</div>';

                        $(elemBtn).find(".export").eq(0).remove();

                        elemBtn.append(exportBtnHTML);                        
                    }
                }
                ...
                function createObjButton(dataObject, myContent, myClass) {
                    var exportButton = "<li data-fileblob='" + dataObject + "' class='exportBtn' role='menuitem' class='exportToExcel' data-type='excel'><a href='javascript:void(0)'>MS-Excel</a></li>";
            ...
            $("li[data-fileblob]")
                .off("click")
                .on("click", function () {
                    var excelData = $(this).data("fileblob").data;

                    var indexReportAction = excelData[0].indexOf('Action');
                    if (indexReportAction > -1) {
                        excelData[0].splice(indexReportAction, 1);
                    }

                    var indexCurrencyPair = excelData[0].indexOf('Currency Pair');
                    if (indexCurrencyPair > -1) {
                        excelData[0].splice(indexCurrencyPair, 1);
                    }

                    var indexFailedValidationCategory = excelData[0].indexOf('Failed Validation Category');
                    if (indexFailedValidationCategory > -1) {
                        excelData[0].splice(indexFailedValidationCategory, 1);
                    }

                    var indexActions = excelData[0].indexOf('Actions');
                    if (indexActions > -1) {
                        excelData[0].splice(indexActions, 1);
                    }

                    var indexId = excelData[0].indexOf('Id');                    
                    if (indexId > -1) {
                        excelData[0].splice(indexId, 1);
                    }
                    
                    if (indexActions > -1 || indexId > -1 || indexFailedValidationCategory > -1 || indexCurrencyPair > -1 || indexReportAction > -1) {
                        for (var i = 1; i < excelData.length; i++) {

                            if (indexReportAction > -1) {
                                excelData[i].splice(indexReportAction, 1);
                            }

                            if (indexCurrencyPair > -1) {
                                excelData[i].splice(indexCurrencyPair, 1);
                            }

                            if (indexFailedValidationCategory > -1) {
                                excelData[i].splice(indexFailedValidationCategory, 1);
                            }
                            if (indexActions > -1) {
                                excelData[i].splice(indexActions, 1);
                            }
                            if (indexId > -1) {
                                excelData[i].splice(indexId, 1);
                            }
                        }
                    }
